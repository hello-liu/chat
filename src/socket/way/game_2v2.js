...
function Game_2v2(table) {
    ...
    this.fapai = function(){
        ...
        player_up.pokers = poker.slice(0,pokerCount)
        player_down.pokers = poker.slice(pokerCount,pokerCount*2)
        player_left.pokers = poker.slice(pokerCount*2,pokerCount*3)
        player_right.pokers = poker.slice(pokerCount*3,pokerCount*4)
    ...
    this.xuanzhu = function(msg){
        ...
        if(sit == table.currentplayer && table.statu == 'major' ){
            ...
            //将底牌发给叫分人
            table['player_'+sit].pokers = table['player_'+sit].pokers.concat( poker.slice(pokerCount*4,pokerCount*4+pokerCountDi) )
            //设置当前状态为扣牌
            table.statu = 'holding'
        }else{
            //无效信息
